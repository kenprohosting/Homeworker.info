...
$employee_id = $_SESSION['freelancer_id'];
$success = '';
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $profile_pic = $freelancer['Profile_pic'] ?? null;
    ...
    if (empty($errors)) {
        $update = $conn->prepare("UPDATE freelancer SET Name = ?, Age = ?, Skills = ?, country = ?, county_province = ?, Language = ?, Education_level = ?, residence_type = ?, Profile_pic = ? WHERE ID = ?");
...
<head>
    ...
    <title>Freelancer Profile - Homeworker Connect</title>
...
<header>
    ...
    <nav class="main-nav">
        <ul class="nav-links">
            <li><span class="user-greeting">Hello, <?= htmlspecialchars($_SESSION['freelancer_name']) ?></span></li>
            <li><a class="nav-btn" href="freelancer_dashboard.php">Dashboard</a></li>
            <li><a class="nav-btn" href="browse_jobs.php">Browse Jobs</a></li>
            <li><a class="nav-btn" href="my_applications.php">My Applications</a></li>
            <li><a class="nav-btn" href="freelancer_logout.php">Logout</a></li>
        </ul>
    </nav>
</header>
...
<div class="form-container">
    ...
    <div class="profile-form">
        ...
        <?php
        $profilePath = $freelancer['Profile_pic'] ?? '';
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                <label>Name:</label>
                <input name="name" value="<?= htmlspecialchars($freelancer['Name'] ?? '') ?>" required>
            </div>

            <div class="form-group">
                <label>Age:</label>
                <input type="number" name="age" value="<?= htmlspecialchars($freelancer['Age'] ?? '') ?>" required>
            </div>

            <div class="form-group">
                <label>Skills:</label>
                <textarea name="skills" required><?= htmlspecialchars($freelancer['Skills'] ?? '') ?></textarea>
            </div>

            <div class="form-group">
                <label>Country:</label>
                <input name="country" value="<?= htmlspecialchars($freelancer['Country'] ?? '') ?>" required>
            </div>

            <div class="form-group">
                <label>County/Province:</label>
                <input name="county_province" value="<?= htmlspecialchars($freelancer['County_province'] ?? '') ?>" required>
            </div>

            <div class="form-group">
                <label>Languages:</label>
                <select name="language" required>
                    <option <?= (isset($freelancer['Language']) && $freelancer['Language'] == 'English') ? 'selected' : '' ?>>English</option>
                    <option <?= (isset($freelancer['Language']) && $freelancer['Language'] == 'Kiswahili') ? 'selected' : '' ?>>Kiswahili</option>
                    <option <?= (isset($freelancer['Language']) && $freelancer['Language'] == 'Both') ? 'selected' : '' ?>>Both</option>
                </select>
            </div>

            <div class="form-group">
                <label>Education Level:</label>
                <input name="education" value="<?= htmlspecialchars($freelancer['Education_level'] ?? '') ?>" required>
            </div>

            <div class="form-group">
                <label>Residence Type:</label>
                <select name="residence_type" required>
                    <option <?= (isset($freelancer['Residence_type']) && $freelancer['Residence_type'] == 'urban') ? 'selected' : '' ?>>urban</option>
                    <option <?= (isset($freelancer['Residence_type']) && $freelancer['Residence_type'] == 'rural') ? 'selected' : '' ?>>rural</option>
                </select>
            </div>
